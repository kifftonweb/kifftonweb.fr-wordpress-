...
function sfsi_plus_check_on_plugin_page($dir_slug, $option_name, $site_url = "")
{
    if(isset($_GET["page"])){
...
function sfsi_plus_has_gallery_plugin($gallery_plugins)
{
    foreach ($gallery_plugins as $key => $gallery_plugin) {
        $sfsi_plus_show_gallery_banner = sfsi_plus_check_on_plugin_page($gallery_plugin['dir_slug'], $gallery_plugin['option_name']);
        if ($sfsi_plus_show_gallery_banner) {
            return $sfsi_plus_show_gallery_banner;
        }
    }
}
function sfsi_plus_has_cache_plugin($optimization_plugins)
{
    foreach ($optimization_plugins as $key => $optimization_plugin) {
        $sfsi_plus_show_optimization_plugin = sfsi_plus_check_on_plugin_page($optimization_plugin['dir_slug'], $optimization_plugin['option_name']);
        if ($sfsi_plus_show_optimization_plugin) {
            return $sfsi_plus_show_optimization_plugin;
        }
    }
}

function sfsi_plus_has_share_count_plugin($sharecount_plugins)
{
    foreach ($sharecount_plugins as $key => $sharecount_plugin) {
        $sfsi_plus_show_share_count_plugin = sfsi_plus_check_on_plugin_page($sharecount_plugin['dir_slug'], $sharecount_plugin['option_name']);
        if ($sfsi_plus_show_share_count_plugin) {
            return $sfsi_plus_show_share_count_plugin;
        }
    }
}

function sfsi_plus_has_google_analytics_plugin($google_analytics)
{
    foreach ($google_analytics as $key => $google_analytic) {
        $sfsi_plus_show_analytics_plugin = sfsi_plus_check_on_plugin_page($google_analytic['dir_slug'], $google_analytic['option_name']);
        if ($sfsi_plus_show_analytics_plugin) {
            return $sfsi_plus_show_analytics_plugin;
        }
    }
}

function sfsi_plus_has_gdpr_plugins($gdpr_plugins)
{
    foreach ($gdpr_plugins as $key => $gdpr_plugin) {
        $sfsi_plus_show_gdpr_plugin = sfsi_plus_check_on_plugin_page($gdpr_plugin['dir_slug'], $gdpr_plugin['option_name']);
        if ($sfsi_plus_show_gdpr_plugin) {
            return $sfsi_plus_show_gdpr_plugin;
        }
    }
}

function sfsi_plus_check_not_show_other_plugin_settings_page($gallery_plugins, $optimization_plugins, $sharecount_plugins, $google_analytics, $gdpr_plugins)
{
    $all_banner = array(
        sfsi_plus_has_gallery_plugin($gallery_plugins),
        sfsi_plus_has_cache_plugin($optimization_plugins),
        sfsi_plus_has_share_count_plugin($sharecount_plugins),
        sfsi_plus_has_google_analytics_plugin($google_analytics),
        sfsi_plus_has_gdpr_plugins($gdpr_plugins)
    );
    $check_any_banner_is_true = in_array(true, $all_banner);

    return $check_any_banner_is_true;
}
function sfsi_plus_social_media_counts_active_onhttp()
{
    // check if user has activated the counts and his homepage has count on fb
    $sfsi_plus_section4            =  unserialize(get_option('sfsi_plus_section4_options', false));
    $sfsi_plus_section1            =  unserialize(get_option('sfsi_plus_section1_options', false));
    if ($sfsi_plus_section1['sfsi_plus_facebook_display'] && $sfsi_plus_section4['sfsi_plus_display_counts'] == "yes" && $sfsi_plus_section4['sfsi_plus_facebook_countsDisplay'] == "yes") {
        return true;
    };
    return false;
}

function sfsi_plus_should_show_loyalty()
{
...
function sfsi_plus_check_banner_criteria($banner, $gallery_plugins, $optimization_plugins, $sharecount_plugins, $google_analytics, $gdpr_plugins, $time)
{
    // var_dump($banner);
    if (
        ($banner['is_active'] == "yes") &&
        $time >= ($banner['timestamp']) && (!sfsi_plus_check_not_show_other_plugin_settings_page($gallery_plugins, $optimization_plugins, $sharecount_plugins, $google_analytics, $gdpr_plugins))
    ) {
        return true;
    }
    return false;
...
function sfsi_plus_get_plugin($dir_slug)
{
